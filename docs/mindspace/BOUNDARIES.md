...
`session` and `review` write only durable sidecar records under
`.mdmind/sessions/` and `.mdmind/reviews/`; current approval records decisions
after digest checks and does not mutate maps.
