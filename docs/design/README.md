# Design Notes

Design notes capture product thinking, implementation rationale, and historical
context. They are not the user manual and they are not the active task backlog.

- [COMMAND_PALETTE.md](COMMAND_PALETTE.md)
- [TERMINAL_EXPERIENCE.md](TERMINAL_EXPERIENCE.md)
- [FOCUSED_VIEWS.md](FOCUSED_VIEWS.md)
- [SPATIAL_CANVAS.md](SPATIAL_CANVAS.md)
- [UX_DESIGN_REVIEW.md](UX_DESIGN_REVIEW.md)
- [IMPORT_AND_INGESTION.md](IMPORT_AND_INGESTION.md)
- [LLM_WIKI_MINDSPACE_STRATEGY.md](LLM_WIKI_MINDSPACE_STRATEGY.md)
- [EXPORT_TARGETS.md](EXPORT_TARGETS.md)
- [CLI_HELP_AND_COMPLETIONS.md](CLI_HELP_AND_COMPLETIONS.md)
- [TEMPLATE_VARIABLES.md](TEMPLATE_VARIABLES.md)
- [DOCUMENTATION_STRATEGY.md](DOCUMENTATION_STRATEGY.md)
- [PHASE1_UX_IMPLEMENTATION.md](PHASE1_UX_IMPLEMENTATION.md)
- [DEEP_LINK_FALLBACK.md](DEEP_LINK_FALLBACK.md)

Active implementation priorities live in Linear. Product requirements that
should be reviewed as product artifacts live in [../product/prds/](../product/prds/).
