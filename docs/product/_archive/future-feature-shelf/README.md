# Future Features

These are roadmap items, not current product behavior.

If a feature is here, it should not be described anywhere else as already real.

## Nearer-Term Ideas

- [checklist-semantics.md](checklist-semantics.md)
- [document-and-branch-stats.md](document-and-branch-stats.md)

## Larger Future Work

- [archive-and-document-ingestion.md](archive-and-document-ingestion.md)
- [docs-site-and-adoption.md](docs-site-and-adoption.md)
- [LLM wiki mindspace layer](../../../design/LLM_WIKI_MINDSPACE_STRATEGY.md)
- [spatial-canvas.md](spatial-canvas.md)
- [template-variables.md](template-variables.md)
- [cli-help-and-completions.md](cli-help-and-completions.md)
