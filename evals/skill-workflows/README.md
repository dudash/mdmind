...
1. `evals/skill-workflows/skill_eval.py --init` creates isolated case directories.
2. A human or agent runs each prompt and writes the requested artifacts into
   `outputs/`.
3. `evals/skill-workflows/skill_eval.py --grade` checks the artifacts with deterministic
   assertions and `mdm` commands.

Run the checked-in golden artifacts:

```bash
python3 evals/skill-workflows/skill_eval.py \
