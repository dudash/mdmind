...
usage() {
  cat <<'EOF'
Usage:
  scripts/release/validate-release-installers.sh --tag v0.2.0 [--repo dudash/mdmind] [--tap-repo ../homebrew-tap] [--check-brew]
...
bash "${repo_root}/scripts/release/render-homebrew-formula.sh" \
  --version "${VERSION}" \
  --repo "${REPO}" \
