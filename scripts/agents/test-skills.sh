...
usage() {
  cat <<'EOF'
Usage:
  scripts/agents/test-skills.sh [--skill mdmind-map-authoring|mdm-cli-inspection] [--dest /tmp/codex-home] [--codex-cmd codex]
...
Examples:
  ...
  scripts/agents/test-skills.sh --dest /tmp/mdmind-skill-test
EOF
}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SKILLS_ROOT="${REPO_ROOT}/plugins/mdmind/skills"
